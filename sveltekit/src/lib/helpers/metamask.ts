...
} from '@kredeum/common/lib/common/types';
...
const metamaskInit = async (): Promise<void> => {
	...
	if (_ethereumProvider) {
		// SET provider
		if (_ethereumProvider !== (window as WindowEthereumProvider).ethereum)
			alert(metamaskMultipleWallets);
		metamaskProvider.set(
			new ethers.providers.Web3Provider((window as WindowExternalProvider).ethereum, 'any')
		);
		...
		try {
			_handleAccounts(
				(await _ethereumProvider.request({ method: 'eth_accounts' })) as Array<string>
			);
		} catch (err) {
