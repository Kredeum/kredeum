<script lang="ts">
	...
	import {
		nftSubListStoreRefresh,
		nftSubListStore
	} from '@kredeum/sveltekit/src/lib/stores//nft/nftSubList';
	import {
		collectionStore,
		collectionStoreRefresh
	...
	const getOwnerSupply = (): number =>
		isAddressNotZero(owner) && $collection?.balancesOf?.has(owner)
			? $collection.balancesOf.get(owner)
			: -1;
	const getTotalSupply = (): number =>
		$collection?.hasOwnProperty('totalSupply') ? $collection.totalSupply : -1;
	...
	const handleDisplayOwnerSupply = (): string =>
		(displayOwnerSupply = isAddressNotZero(owner)
			? getOwnerSupply() >= 0
				? String(getOwnerSupply())
				: '?'
			: '');
...
<div class="row">
	<div class="col col-sm">
		{#if !end}
			<button class="btn btn-default" on:click={more} title="      Click to View more NFTs"
