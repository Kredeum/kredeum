<script lang="ts">
	...
	import { refPage2UrlHash, refPageFromUrlHash } from '@kredeum/sveltekit/src/lib/helpers//refPage';
	...
	onMount(async () => {
		...
		account = isAddressNotZero(_refHash.account)
			? _refHash.account
			: $metamaskSignerAddress || constants.AddressZero;
		...
		<div class="row alignbottom">
			...
			<div class="col col-xs-12 col-sm-3 kre-copy-ref-container">
				<CollectionSelect
					...
					bind:address
					{account}
					bind:refreshing={refreshingCollections}
				/>
			</div>
		...
		{#if isCollection({ chainId, address })}
			{#if tokenIdCount(tokenID) == 1}
				<h2 class="m-b-20 return">
					...
					<span on:click={resetTokenID} on:keydown={resetTokenID} class="link"
