<script lang="ts">
	...
	import {
		collectionSubListRefresh,
		collectionSubListStore
	} from '@kredeum/sveltekit/src/lib/stores//collection/collectionSubList';
	import {
		collectionDefaultRefresh,
		collectionDefaultSetOne,
		collectionDefaultSubStore
	} from '@kredeum/sveltekit/src/lib/stores//collection/collectionDefault';
	...
	// UTILITIES
	const _setCollectionFromEvent = (evt: Event) =>
		_setCollection((evt.target as HTMLInputElement).value);
	const _explorerCollectionUrl = (collection: string): string =>
		explorerCollectionUrl(chainId, collection);
