<script lang="ts">
	...
	import { nftPrice } from '@kredeum/common/lib/nft/nft';

	import { buyNft } from '@kredeum/common/lib/nft/nft-buy';
	import {
		explorerNftUrl,
		explorerTxUrl,
		explorerTxLog,
	...
	const buyConfirm = async () => {
		...
		if (!buyTxHash)
			return _buyError(`ERROR while sending transaction... ${JSON.stringify(buyTxResp, null, 2)}`);
			...
			<div class="modal-body">
				<div>
					{#if buying == S1_CONFIRM}
						...
						<div class="txtright">
							{#if signer}
								<button class="btn btn-default btn-sell" type="submit" on:click={() => buyConfirm()}
									>Buy</button
								>
							{:else}
					...
					{#if buyTxHash}
						<div class="flex">
							<a
								class="link"
								href={explorerTxUrl(chainId, buyTxHash)}
								target="_blank"
								rel="noreferrer">{textShort(buyTxHash)}</a
							>
						</div>
					{/if}
