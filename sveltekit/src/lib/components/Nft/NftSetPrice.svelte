<script lang="ts">
	...
	const tokenSetPriceTx = async (price: BigNumber): Promise<void> => {
		...
		if (!Boolean(txReceipt.status))
			return _tokenSetPriceError(`ERROR returned by transaction ${txReceipt}`);
...
{#if tokenSettingPrice == S1_CONFIRM}
	...
	{#if !collectionApproved}
		<div class="section">
			<div class="form-field kre-warning-msg">
				<p>
					By completing this listing you allow this AutoMarket collection to manage the exchange of
					your NFTs
					<a
						class="link"
	...
	<div class="txtright">
		{#if nftOnSale($nft)}
			<button class="btn btn-default btn-remove" type="submit" on:click={removeFromSale}
		...
		<button
			...
			type="submit"
			on:click={() => tokenSetPriceConfirm(priceInput)}
		>
			{#if nftOnSale($nft)}
				Modify Listing
			{:else}
...
{#if tokenSetPriceTxHash}
	<div class="flex">
		<a
			class="link"
			href={explorerTxUrl(chainId, tokenSetPriceTxHash)}
			target="_blank"
			rel="noreferrer">{textShort(tokenSetPriceTxHash)}</a
		>
	</div>
{/if}
