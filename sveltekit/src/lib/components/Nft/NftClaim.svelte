<script lang="ts">
	...
	import { nftMint, nftClaimed } from '@kredeum/common/lib/nft/nft-mint';
	import { cidToInt } from '@kredeum/common/lib/common/cid';
			...
			<div>
				...
				{:else}
					<div class="titre overflow-ellipsis">
						<i class="fas fa-hand-holding-usd" /> Claim this NFT #{textShort(tokenID, 4)} on another
						network ?
					</div>
				...
				{#if claimTxHash}
					<div class="flex">
						<a
							class="link"
							href={explorerTxUrl(chainId, claimTxHash)}
							target="_blank"
							rel="noreferrer">{textShort(claimTxHash)}</a
						>
					</div>
				{/if}
			</div>
