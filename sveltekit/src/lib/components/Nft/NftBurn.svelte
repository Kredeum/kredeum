<script lang="ts">
	...
	import { burnNft } from '@kredeum/common/lib/nft/nft-burn';
	import {
		ADDRESS_DEAD,
		explorerNftUrl,
		explorerTxUrl,
	...
	import { collectionBurnable } from '@kredeum/common/lib/collection/collection-get';
	...
	import { nftStoreRemove } from '@kredeum/sveltekit/src/lib/stores//nft/nft';
	...
	const burnConfirm = async () => {
		...
		if (!burnTxHash)
			return _burnError(`ERROR while sending transaction... ${JSON.stringify(txResp, null, 2)}`);
			...
			<div>
				...
				{:else if burning == S1_CONFIRM_BURN}
					<div class="section">
						<div class="form-field kre-warning-msg">
							<p>
								<i class="fas fa-exclamation-triangle fa-left c-red" /> Be carefull, you're about to
								burn this NFT #{tokenID}
							</p>
				...
				{:else if burning == S2_CONFIRM_TRANSFER}
					<div class="section">
						<div class="form-field kre-warning-msg">
							<p>
								<i class="fas fa-exclamation-triangle fa-left c-red" /> This NFT #{tokenID} can't be
								burned!
							</p>
							<p>
								But you can get rid of it anyway, by transfering it to an unusable "dEaD address"
							</p>
							<p>{ADDRESS_DEAD}</p>
							<p>This operation is irreversible !!!</p>
						</div>
				...
				{#if burnTxHash}
					<div class="flex">
						<a
							class="link"
							href={explorerTxUrl(chainId, burnTxHash)}
							target="_blank"
							rel="noreferrer">{textShort(burnTxHash)}</a
						>
					</div>
				{/if}
				{#if burnError}
