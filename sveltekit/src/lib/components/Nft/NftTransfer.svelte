<script lang="ts">
	...
	import { transferNft } from '@kredeum/common/lib/nft/nft-transfer';

	import { metamaskSignerAddress } from '@kredeum/sveltekit/src/lib/stores//metamask';
	import { nftStore, nftStoreRefresh } from '@kredeum/sveltekit/src/lib/stores//nft/nft';

	import InputEthAddress from '../Input/InputEthAddress.svelte';
	import { nftRoyaltyMinimum } from '@kredeum/common/lib/nft/nft';
	...
	const transferConfirm = async () => {
		const transferTxRespYield = transferNft(
			chainId,
			address,
			tokenID,
		...
		if (!transferTxHash)
			return _transferError(
				`ERROR while sending transaction... ${JSON.stringify(transferTxResp, null, 2)}`
			);
		...
		if (!Boolean(txReceipt.status))
			return _transferError(`ERROR returned by transaction ${txReceipt}`);
			...
			<div>
				...
				{#if transfering == S1_CONFIRM}
					...
					<div class="section">
						<div class="form-field">
							<InputEthAddress
								bind:ethAddress={destinationAddress}
								placeholder={'destinator address'}
							/>
						</div>
					</div>
					...
					{#if nftRoyaltyMinimum($nft).gt(0)}
						<div class="section">
							<div class="form-field kre-warning-msg">
								<p>
									<i class="fas fa-exclamation-triangle fa-left c-red" /> Be carefull, you're about
									to transfer this NFT #{tokenID} which requires minimum royalty payment. That means
									you have, in any case, to pay the minimal Royalty amount of {displayEther(
										chainId,
										nftRoyaltyMinimum($nft)
									)} to transfer it.
								</p>
							</div>
						</div>
					...
					<div class="txtright">
						<button class="btn btn-default btn-sell" type="submit" on:click={transferConfirm}
							>Transfer</button
						>
					</div>
				...
				{#if transferTxHash}
					<div class="flex">
						<a
							class="link"
							href={explorerTxUrl(chainId, transferTxHash)}
							target="_blank"
							rel="noreferrer">{textShort(transferTxHash)}</a
						>
					</div>
				{/if}
				{#if transferError}
