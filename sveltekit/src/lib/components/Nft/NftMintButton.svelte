<script lang="ts">
  ...
  import { S0_START, S5_MINTED, nftMintTexts } from "@kredeum/sveltekit/src/lib/helpers/nftMint";
  import { getDappUrl } from "@kredeum/common/lib/common/config";
  ...
  const view = () => nft && (location.href = getDappUrl(chainId, nft));
...
<div class="kredeum-mint">
  {#if src && chainId && address && signer}
    ...
    {:else if minting === S5_MINTED}
      <button on:click|preventDefault={view} class="btn btn-small btn-view" title="View in Explorer"> VIEW NFT </button>
    {:else}
