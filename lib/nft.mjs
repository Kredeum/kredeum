...
nft.init = function (chainId) {
  const network = networksKRE.find((nw) => Number(nw.chainId) === Number(chainId));

  if (network) {
    network.KRE = KRE.ADDRESS[network.chainName][0];
    if (network.KRE) {
      const provider = new ethers.providers.JsonRpcProvider(network.rpcUrls[0]);
      contractKRE = new ethers.Contract(network.KRE, KRE.ABI, provider);
    }
  }
  return network;
};
...
nft.get = async function (index) {
  const nft = {};
  try {
    nft.tokenId = (await contractKRE.tokenByIndex(index)).toString();
    nft.ownerOf = await contractKRE.ownerOf(nft.tokenId);
    nft.tokenURI = await contractKRE.tokenURI(nft.tokenId);
...
nft.listContract = async function () {
  const totalSupply = (await contractKRE.totalSupply()).toNumber();
...
nft.Mint = async function (signer, urlJson) {
  ...
  const tx1 = await contractKRE.connect(signer).addUser(address, urlJson);
