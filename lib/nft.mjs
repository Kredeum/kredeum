...
nft.listTheGraph = async function (chainId) {
  let nfts = [];
...
nft.listContract = async function (chainId) {
...
nft.list = async function (chainId, type = 'thegraph') {
  const nftList = new Map();
  let nfts;

  if (type === 'thegraph') nfts = await nft.listTheGraph(chainId);
  else nfts = await nft.listContract(chainId);
