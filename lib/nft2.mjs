...
nft.fetch = async function (uri, config = {}) {
  let json = {};
  try {
    json = await (await fetch(nft.tokenURI, config)).json();
  } catch (e) {
    console.error("nft.fetch ERROR", uri, e);
  }
  return json;
};
...
nft.get = async function (index) {
  ...
  try {
    ...
    // nft.tokenJson = await nft.fetch(nft.tokenURI);
...
nft.listTheGraph = async function () {
  ...
  const answerGQL = (await (await fetch(network.subgraphUrl, config)).json()).data;
