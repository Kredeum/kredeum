...
class OpenNFTs {
  ...
  async listNFTsFromContract() {
    ...
    try {
      ...
      for (let index = 0; index < Math.min(nbTokens, this.limit); index++) {
        // console.log("OpenNFTs.listNFTsFromContract item", index + 1);
        tokens[index] = await this.getNFTFromContract(this.owner, index);
      }
    } catch (e) {
  ...
  async listNFTsFromTheGraph() {
    ...
    // console.log(query);
    const answerGQL = await this.fetchGQL(this.urlSubgraph(), query);
  ...
  async listContracts() {
    ...
    if (this.urlSubgraph()) contracts = await this.listContractsFromTheGraph();
    else {
  ...
  async Mint(_signer, _urlJson) {
    ...
    const tx1 = await this.cont;
    ract.connect(_signer).mintNFT(address, _urlJson);
