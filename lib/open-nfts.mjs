...
class OpenNFTs {
  ...
  async initContract(_chainId, _address) {
    ...
    this.setContract(_chainId, _address);
  ...
  async fetchCov(_path) {
    const loginPass = "ckey_9d7db85996c542e7bc3bc2a24ce:";
  ...
  async listNFTs(_owner, _type = "subgraph") {
    // console.log("listNFTs", _owner, _type);
    let tokens = [];
  ...
  listNFTsFromCache(_owner) {
    const chainContract = this.getContract()?.toLowerCase();
    console.log("listNFTsFromCache", _owner, chainContract);
