...
import { configNetworks, configContracts, abis, getProvider, chainIdDefault } from "./config.mjs";
...
class ClassOpenNfts {
  ...
  setConfigContract(_chainId, _address) {
    ...
    if (_address) {
      this.configContract = configContracts.find(
        (_contract) =>
          ...
          _contract.network === this.configNetwork?.chainName
      ) || {
        address: _address,
    ...
    // console.log("configContract", this.configContract);
    {
      // SET subgraph.url
      this.subgraph = {
        url: this.configContract.subgraph?.url || this.configNetwork.subgraph?.url,
        active: this.configContract.subgraph?.active || this.configNetwork.subgraph?.active
      };
  ...
  async setContract(_chainId, _address) {
    if (_address) this.setConfigContract(_chainId, _address);
  ...
  async Mint(_signer, _urlJson) {
    ...
    return { chainId: this.configNetwork.chainId, contract: this.currentContract.address, tokenID };
  }
}
