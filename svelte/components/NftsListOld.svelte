<script lang="ts">
  ...
  import { nftList, nftListFromCache, nftListTokenIds } from "lib/knft-list";
  import { nftGetMetadata, nftGetImageLink } from "lib/knft-get-metadata";
  ...
  import KredeumGetNft from "./NftGetOld.svelte";
  ...
  const _nftsListFromLib = async (_chainId: number, _collection: Collection, _owner: string) => {
    cacheClear($chainId, collection.address);
    mores = [];
    NFTs = new Map();
