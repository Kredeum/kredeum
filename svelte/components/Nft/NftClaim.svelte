<script lang="ts">
  ...
  import { getOpenMulti, explorerNftUrl, explorerTxUrl, textShort, ipfsLinkToUrlHttp } from "lib/kconfig";
    ...
    if ($metamaskSigner && $nft) {
      ...
      try {
        if (!$nft.tokenURI) claimingError = "No metadata found on this NFT";
          ...
          const cid = await nftStorage.pinUrl(ipfsLinkToUrlHttp($nft.tokenURI));
