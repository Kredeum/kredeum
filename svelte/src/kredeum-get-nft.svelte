<script lang="ts">
  ...
  const shortcode = async (nft: Nft) => {
    const data = `[kredeum_sell chain="${nft.chainName}" collection="${nft.collection}" tokenid="${nft.tokenID}" ipfs="${nft.ipfs}"]${nftName(nft)}[/kredeum_sell]`;
...
<div id="table-drop-{index}" class="table-row table-drop" class:closed="{!more}" style="height: {more ? `${more}px` : 'auto'};">
    ...
    <div id="description-{index}" class="description">
      ...
      <ul class="steps">
        ...
        <li class="complete">
          ...
          <div class="flex">
            <a class="link" href="{explorerCollectionUrl($chainId, nft?.collection)}" target="_blank">{getShortAddress(nft.collection, 15)}</a>
          </div>
        </li>
