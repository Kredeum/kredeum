<script lang="ts">
  ...
  import NftProperties from "../Nft/NftProperties.svelte";
  import {
    collectionPrice,
  ...
  const mintConfirm = async () => {
    ...
    mintingTxResp = await nftMint(chainId, address, storageJson, $metamaskSignerAddress, inputPrice);
    if (!mintingTxResp)
    ...
    mintedNft = await nftMinted(chainId, address, mintingTxResp, storageJson, $metamaskSignerAddress);
