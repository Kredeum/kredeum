	...
	add_action(
		...
		function ( $hook ) {
			if ( 'nfts_page_storage_settings' === $hook || 'toplevel_page_nfts' === $hook || 'upload.php' === $hook || 'edit.php' === $hook ) {
				wp_enqueue_script( 'kredeum_nfts', plugin_dir_url( __FILE__ ) . 'assets/js/kredeum-nfts.js', array(), KREDEUM_NFTS_VERSION, true );
				wp_register_style( 'kredeum_nfts_css', plugin_dir_url( __FILE__ ) . 'assets/js/kredeum-nfts.css', KREDEUM_NFTS_VERSION, true );
				wp_enqueue_style( 'kredeum_nfts_css' );
			}
			if ( 'nfts_page_storage_settings' === $hook || 'toplevel_page_nfts' === $hook || 'upload.php' === $hook || 'edit.php' === $hook ) {
				wp_register_style( 'kredeum_nfts_front_css', plugin_dir_url( __FILE__ ) . 'assets/css/front.css', KREDEUM_NFTS_VERSION, true );
...
define( 'IPFS_ENDPOINT', '{{storage.ipfs.apiEndpoint}}' );
