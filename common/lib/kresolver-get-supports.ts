...
import { CollectionSupports } from "@lib/ktypes";
import { abis } from "@lib/kabis";
import { interfaceId } from "@lib/kconfig";
import { resolverGetContract } from "@lib/kresolver-get";

const resolverGetSupports = (checks: Array<boolean>): CollectionSupports => {
  ...
  if (!(supports.IERC165 && !checks[0] && i == 22)) throw "ERROR resolverGetSupports";
...
const resolverGetCollectionSupports = async (
  ...
  // console.log("resolverGetCollectionSupports", address);

  const nftsResolver = resolverGetContract(chainId, provider);

  /// ids to check
  const ids = [
    interfaceId(abis["IOpenNFTs"]),
  ...
  const checks = await nftsResolver.checkSupportedInterfaces(address, true, ids);

  const supports = resolverGetSupports(checks);

  return supports;
};

export { resolverGetCollectionSupports, resolverGetSupports };
