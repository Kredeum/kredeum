...
async function* transferNft(
  ...
  if (!(chainId && address && address != ZeroAddress && tokenID && to && from)) return {};
  ...
  if (collection.supports?.IOpenAutoMarket) {
    ...
    if (collection.minimal) {
      [, , minimumRoyalty] = await (contract as unknown as OpenAutoMarket).getTokenRoyalty(tokenID);
    }
    // console.log("minimumRoyalty", minimumRoyalty);

    txResp = await (contract as unknown as IOpenAutoMarket).gift(to, tokenID, { value: String(minimumRoyalty) });
  } else if (collection.supports?.IERC721) {
    // console.log("transferNft IERC721 safeTransferFrom", from, to, tokenID);
    txResp = await (contract as unknown as IERC721)["safeTransferFrom(address,address,uint256)"](from, to, tokenID);
  } else if (collection.supports?.IERC1155) {
    txResp = await (contract as unknown as IERC1155).safeTransferFrom(from, to, tokenID, 1, "0x00");
  }
  if (!txResp) return {};
