...
abstract contract OpenERC2981 is IERC2981, IOpenReceiverInfos, OpenERC165 {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (royalty.minimum > 0) {
            ...
            uint256 minimumRoyaltyAmount = royalty.minimum < defaultRoyaltyAmount
                ? royalty.minimum
...
interface IOpenMarketable is IOpenReceiverInfos {
    ...
    function getTokenRoyalty(uint256 tokenID) external view returns (ReceiverInfos memory receiver);
}
...
abstract contract OpenMarketable is IOpenMarketable, OpenERC721, OpenERC173, OpenERC2981, OpenGuard {
    ...
    function setTokenRoyalty(
        ...
        address receiver,
        uint96 fee
    ) public override(IOpenMarketable) existsToken(tokenID) onlyOwner onlyTokenOwnerOrApproved(tokenID) {
    ...
    function getDefaultRoyalty() public view override(IOpenMarketable) returns (ReceiverInfos memory receiver) {
    ...
    function _createReceiverInfos(address receiver, uint96 fee) internal view returns (ReceiverInfos memory) {
