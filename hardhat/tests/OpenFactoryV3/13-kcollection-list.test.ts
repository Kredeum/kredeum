...
import type { OpenNFTsV4, OpenFactoryV3 } from "@soltypes/contracts/next";

import type { NetworkType } from "@lib/common/ktypes";

import { collectionList } from "@lib/collection/kcollection-list";
...
describe.skip("13 List contracts lib", function () {
  ...
  beforeEach(async () => {
    ...
    nftsFactory = (await ethers.getContract("OpenFactoryV3", signer)) as unknown as OpenFactoryV3;
    expect(nftsFactory.address).to.be.properAddress;

    await nftsFactory.connect(deployer).setTemplate("generic", openNFTsV4.address);
  });

  it("List with OpenFactoryV3", async function () {
    if (chainId !== 31337) {
